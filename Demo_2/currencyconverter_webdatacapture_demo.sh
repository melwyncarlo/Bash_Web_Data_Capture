...
function_getrate()
{
	...
	hcam=`echo "scale=$decpts; ($ccam*${exchangerateArray[ccid]})/${exchangerateArray[hcid]}" | bc`
	if [[ "${hcam:0:1}" == "." ]]; then
		hcam="0""$hcam"
	fi
}
...
function_readfile "$curconvSavedir/mfc_time_code.txt" tmpmsg2
...
if [[ "$tmpmsg1" != "1" ]]; then
	...
	if [[ "$tmpmsg2" == *"0s"* ]]; then
		...
		function_readfile "$curconvSavedir/mfc_time_text.txt" tmpmsg2
		ltime="$tmpmsg2 old."
	fi
									...
									do
										...
										if [ $? = 0 ]; then
											...
											[[ "${inputmsg:$tmpLen2:1}" != "." ]]; then
												...
												"$clientmoderesultid" "$inputmsg" tmpmsg1 "2"
												tmpmsg2="\n\n"
												...
												tmpmsg2+=" =  "
												function_getrate "$hostcountryid" \
												"$clientmoderesultid" "1.0" tmpmsg1 "10"
												tmpmsg2+="$tmpmsg1 "
												tmpmsg2+="${currency_code[$hostcountryid]} \n\n\n"
												tmpmsg2+="\n\n  "
